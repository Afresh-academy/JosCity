  ...
  try {
    ...
    try {
      ...
      const [userResult] = await connection.execute(
        `INSERT INTO users 
         (firstname, lastname, gender, phone_number, nin_number, user_email, user_password, address, account_status) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
        [
          firstname,
          lastname,
      ...
      await connection.execute(
        `INSERT INTO pending_approvals (user_id, firstname, lastname, nin_number, user_email) 
         VALUES (?, ?, ?, ?, ?)`,
        [userResult.insertId, firstname, lastname, nin_number, email]
      );
...
exports.getPendingApprovals = async (req, res) => {
  try {
    const [pendingUsers] = await db.execute(
      `SELECT u.user_id, u.firstname, u.lastname, u.gender, u.phone_number, u.nin_number, 
...
exports.approveAccount = async (req, res) => {
  try {
    ...
    const [users] = await db.execute(
      "SELECT user_email, firstname, lastname FROM users WHERE user_id = ?",
      [user_id]
    );
...
exports.signIn = async (req, res) => {
  try {
    ...
    // Find user
    const [users] = await db.execute(
      `SELECT user_id, user_email, user_password, firstname, lastname, gender,
...
exports.forgetPassword = async (req, res) => {
  try {
    ...
    const [users] = await db.execute(
      'SELECT user_id, firstname, lastname FROM users WHERE user_email = ? AND account_status = "approved"',
      [email]
    );
